    ...
    PdfChunk(Chunk chunk, PdfAction action) {
        ...
        if (baseFont == null) {
            ...
            if ((style & Font.BOLD) != 0)
                attributes.put(Chunk.TEXTRENDERMODE, new Object[] {
                        Integer.valueOf(PdfContentByte.TEXT_RENDER_MODE_FILL_STROKE), Float.valueOf(size / 30f), null });
            // italic simulation
    ...
    PdfChunk split(float width) {
        ...
        if (ft.getFontType() == BaseFont.FONT_TYPE_CJK && ft.getUnicodeEquivalent(' ') != ' ') {
            ...
            while (currentPosition < length) {
                ...
                if (surrogate)
                    currentWidth += font
                            .width(Utilities.convertToUtf32(valueArray[currentPosition], valueArray[currentPosition + 1]));
                else
        ...
        if (hyphenationEvent != null && lastSpace >= 0 && lastSpace < currentPosition) {
            ...
            if (wordIdx > lastSpace) {
                String pre = hyphenationEvent.getHyphenatedWordPre(value.substring(lastSpace, wordIdx), font.getFont(),
                        font.size(), width - lastSpaceWidth);
