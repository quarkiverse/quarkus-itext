...
public class BidiLine {
    ...
    public boolean getParagraph(int runDirection) {
        ...
        if (runDirection == PdfWriter.RUN_DIRECTION_LTR || runDirection == PdfWriter.RUN_DIRECTION_RTL) {
            ...
            BidiOrder order = new BidiOrder(text, 0, totalTextLength,
                    (byte) (runDirection == PdfWriter.RUN_DIRECTION_RTL ? 1 : 0));
    ...
    public PdfLine processLine(float leftX, float width, int alignment, int runDirection, int arabicOptions) {
        ...
        if (lastValidChunk == null) {
            ...
            if (surrogate)
                ++currentChar;
            return new PdfLine(0, originalWidth, 0, alignment, false, createArrayOfPdfChunks(currentChar - 1, currentChar - 1),
                    isRTL);
        }
        if (currentChar >= totalTextLength) {
            ...
            if (he != null) {
                ...
                if (word != null) {
                    ...
                    if (pre.length() > 0) {
                        ...
                        return new PdfLine(0, originalWidth, testWidth - lastValidChunk.font().width(pre), alignment, false,
                                createArrayOfPdfChunks(oldCurrentChar, word[0] - 1, extra), isRTL);
                    }
                }
            }
        }
        if (lastSplit == -1 || lastSplit >= newCurrentChar) {
            ...
            return new PdfLine(0, originalWidth, width + getWidth(newCurrentChar + 1, currentChar - 1), alignment, false,
