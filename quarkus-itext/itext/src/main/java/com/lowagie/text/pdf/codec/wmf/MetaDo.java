...
public class MetaDo {
                ...
                case META_STRETCHDIB: {
                    ...
                    try {
                        ...
                        bmp.setAbsolutePosition(xDest - destWidth * xSrc / srcWidth,
                                yDest + destHeight * ySrc / srcHeight - bmp.getScaledHeight());
                        cb.addImage(bmp);
                        cb.restoreState();
                    } catch (Exception e) {
                        // empty on purpose
    ...
    public boolean isNullStrokeFill(boolean isRectangle) {
        ...
        boolean isBrush = (style == MetaBrush.BS_SOLID
                || (style == MetaBrush.BS_HATCHED && state.getBackgroundMode() == MetaState.OPAQUE));
    ...
    public void strokeAndFill() {
        ...
        if (penStyle == MetaPen.PS_NULL) {
            ...
            boolean isBrush = (brushStyle == MetaBrush.BS_SOLID
                    || (brushStyle == MetaBrush.BS_HATCHED && state.getBackgroundMode() == MetaState.OPAQUE));
            if (isBrush) {
                if (state.getPolyFillMode() == MetaState.ALTERNATE)
