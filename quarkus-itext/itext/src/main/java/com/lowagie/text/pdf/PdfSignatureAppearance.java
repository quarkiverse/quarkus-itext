...
public class PdfSignatureAppearance {
    ...
    public PdfTemplate getAppearance() throws DocumentException {
        ...
        if (app[2] == null) {
            ...
            if (render == SignatureRenderNameAndDescription) {
                ...
                ct2.setSimpleColumn(new Phrase(signedBy, font), signatureRect.getLeft(), signatureRect.getBottom(),
                        signatureRect.getRight(), signatureRect.getTop(), signedSize, Element.ALIGN_LEFT);
            ...
            ct.setSimpleColumn(new Phrase(text, font), dataRect.getLeft(), dataRect.getBottom(), dataRect.getRight(),
                    dataRect.getTop(), size, Element.ALIGN_LEFT);
            ct.go();
        }
        ...
        if (app[4] == null && !acro6Layers) {
            ...
            ColumnText ct = new ColumnText(t);
            ct.setRunDirection(runDirection);
            ct.setSimpleColumn(new Phrase(text, font), MARGIN, 0, rect.getWidth() - MARGIN, rect.getHeight() - MARGIN, size,
                    Element.ALIGN_LEFT);
            ct.go();
        }
        int rotation = writer.reader.getPageRotation(page);
    ...
    public static float fitText(Font font, String text, Rectangle rect, float maxFontSize, int runDirection) {
        try {
            ...
            ct.setSimpleColumn(ph, rect.getLeft(), rect.getBottom(), rect.getRight(), rect.getTop(), maxFontSize,
                    Element.ALIGN_LEFT);
            ct.setRunDirection(runDirection);
            status = ct.go(true);
            ...
            for (int k = 0; k < 50; ++k) { //just in case it doesn't converge
                ...
                ct = new ColumnText(null);
                font.setSize(size);
                ct.setSimpleColumn(new Phrase(text, font), rect.getLeft(), rect.getBottom(), rect.getRight(), rect.getTop(),
    ...
    public void preClose(HashMap exclusionSizes) throws IOException, DocumentException {
        ...
        if (cryptoDictionary == null) {
            ...
            PdfLiteral lit = new PdfLiteral(
                    (contents.toString().length() + (PdfName.ADOBE_PPKLITE.equals(getFilter()) ? 0 : 64)) * 2 + 2);
            ...
            for (Iterator it = update.getKeys().iterator(); it.hasNext();) {
                ...
                if (bf.size() > lit.getPosLength())
                    throw new IllegalArgumentException(
                            "The key " + key.toString() + " is too big. Is " + bf.size() + ", reserved " + lit.getPosLength());
                if (tempFile == null)
