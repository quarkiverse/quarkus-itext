...
public class PdfDocument extends Document {
            ...
            try {
                ...
                if (!documentFileAttachment.isEmpty()) {
                    names.put(PdfName.EMBEDDEDFILES,
                            writer.addToBody(PdfNameTree.writeTree(documentFileAttachment, writer)).getIndirectReference());
                }
                if (names.size() > 0)
                ...
                case Element.ANNOTATION: {
                    ...
                    if (line != null)
                        rect = new Rectangle(annot.llx(indentRight() - line.widthLeft()),
                                annot.lly(indentTop() - currentHeight), annot.urx(indentRight() - line.widthLeft() + 20),
    ...
    public boolean newPage() {
        ...
        try {
            ...
            if (text.size() > textEmptySize)
                ...
                text = null;
            writer.add(page,
                    new PdfContents(writer.getDirectContentUnder(), graphics, text, writer.getDirectContent(), pageSize));
        ...
        if (separatorCount > 0) {
            ...
            if (line.isNewlineSplit() && line.widthLeft() >= (lastBaseFactor * (ratio * numberOfSpaces + lineLen - 1))) {
                ...
                float baseFactor = (numberOfSpaces == 0 && ratio == PdfWriter.NO_SPACE_CHAR_RATIO)
                        ? 0f
                        : width / (ratio * numberOfSpaces + lineLen - 1);
                ...
                if (chunk.isStroked()) {
                    ...
                    if (chunk.isAttribute(Chunk.REMOTEGOTO)) {
                        ...
                        if (obj[1] instanceof String)
                            ...
                            remoteGoto(filename, ((Integer) obj[1]).intValue(), xMarker, yMarker, xMarker + width - subtract,
                                    yMarker + chunk.font().size());
                    }
                    if (chunk.isAttribute(Chunk.LOCALGOTO)) {
                        ...
                        if (nextChunk == null)
                            subtract += hangingCorrection;
                        localGoto((String) chunk.getAttribute(Chunk.LOCALGOTO), xMarker, yMarker, xMarker + width - subtract,
                                yMarker + chunk.font().size());
                    }
                    ...
                    if (chunk.isAttribute(Chunk.PDFANNOTATION)) {
                        ...
                        annot.put(PdfName.RECT,
                                new PdfRectangle(xMarker, yMarker + descender, xMarker + width - subtract, yMarker + ascender));
    ...
    boolean fitsPage(PdfPTable table, float margin) {
        ...
        // ensuring that a new line has been started.
        ensureNewLine();
        return table.getTotalHeight() + ((currentHeight > 0) ? table.spacingBefore() : 0f) <= indentTop() - currentHeight
                - indentBottom() - margin;
    }
