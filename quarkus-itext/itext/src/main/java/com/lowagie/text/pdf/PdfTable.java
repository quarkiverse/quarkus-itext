...
public class PdfTable extends Rectangle {
     ...
     * <p>
     * <b>Pre-requisite</b>: the object must have been built with the parameter <code>supportUpdateRowAdditions</code> equals to
     * true.
     */
        ...
        for (Iterator rowIterator = table.iterator(); rowIterator.hasNext();) {
            ...
            if (row.isEmpty()) {
                ...
                for (int i = 0; i < row.getColumns(); i++) {
                    ...
                    if (cell != null) {
                        currentCell = new PdfCell(cell, rowNumber + prevRows, positions[i], positions[i + cell.getColspan()],
                        ...
                        try {
                            if (offsets[rowNumber] - currentCell.getHeight()
                                    - cellpadding() < offsets[rowNumber + currentCell.rowspan()]) {
                                offsets[rowNumber + currentCell.rowspan()] = offsets[rowNumber] - currentCell.getHeight()
                                        - cellpadding();
                            }
