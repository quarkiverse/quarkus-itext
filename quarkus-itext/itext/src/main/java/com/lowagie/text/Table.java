...
public class Table extends Rectangle implements LargeElement {
    ...
    public void addCell(Cell aCell, Point aLocation) throws BadElementException {
        ...
        if (!isValidLocation(aCell, aLocation))
            throw new BadElementException("Adding a cell at the location (" + aLocation.x + "," + aLocation.y
                    + ") with a colspan of " + aCell.getColspan() + " and a rowspan of " + aCell.getRowspan()
    ...
    public void insertTable(Table aTable, Point aLocation) {
        ...
        if (aLocation.y > columns) {
            throw new IllegalArgumentException(
                    "insertTable -- wrong columnposition(" + aLocation.y + ") of location; max =" + columns);
        }
                ...
                for (j = 0; j < columns; j++) {
                    ...
                    } else // copy others values
                    {
                        ...
                        if (Cell.class.isInstance(aElement)) {
                            ...
                            ((Cell) aElement)
                                    .setRowspan(((Cell) ((Row) rows.get(i)).getCell(j)).getRowspan() + lDummyHeights[i] - 1);
                            ((Cell) aElement)
                                    .setColspan(((Cell) ((Row) rows.get(i)).getCell(j)).getColspan() + lDummyWidths[j] - 1);
     ...
     * <P>
     * <UL>
     * <LI>rowspan/colspan not beyond borders
     * <LI>spanned cell don't overlap existing cells
     * </UL>
     *
