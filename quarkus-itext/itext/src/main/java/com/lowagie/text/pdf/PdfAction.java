...
public class PdfAction extends PdfDictionary {
    ...
    /**
     * Construct a new <CODE>PdfAction</CODE> of Subtype URI that accepts the x and y coordinate of the position that was
     * clicked.
     *
     * @param url
     * @param isMap
     ...
     * @param isName if true sets the destination as a name, if false sets it as a String
     * @param newWindow open the document in a new window if <CODE>true</CODE>, if false the current document is replaced by the
     *        new document.
     ...
     */
    public static PdfAction gotoEmbedded(String filename, PdfTargetDictionary target, String dest, boolean isName,
            boolean newWindow) {
        if (isName)
            ...
            if (o instanceof PdfIndirectReference)
                ...
                if (s.equalsIgnoreCase("on"))
                    ...
                    throw new IllegalArgumentException(
                            "A string '" + s + " was passed in state. Only 'ON', 'OFF' and 'Toggle' are allowed.");
                a.add(name);
            } else
