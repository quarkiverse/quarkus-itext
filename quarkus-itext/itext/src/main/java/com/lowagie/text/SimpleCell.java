...
public class SimpleCell extends Rectangle implements PdfPCellEvent, TextElementArray {
    ...
    public void addElement(Element element) throws BadElementException {
        if (cellgroup) {
            if (element instanceof SimpleCell) {
                ...
                throw new BadElementException(
                        "You can only add cells to rows, no objects of type " + element.getClass().getName());
            }
        }
        if (element.type() == Element.PARAGRAPH
            ...
            throw new BadElementException(
                    "You can't add an element of type " + element.getClass().getName() + " to a SimpleCell.");
        }
    }
    ...
    public void cellLayout(PdfPCell cell, Rectangle position, PdfContentByte[] canvases) {
        ...
        if (Float.isNaN(sp_bottom))
            sp_bottom = 0f;
        Rectangle rect = new Rectangle(position.getLeft(sp_left), position.getBottom(sp_bottom), position.getRight(sp_right),
                position.getTop(sp_top));
