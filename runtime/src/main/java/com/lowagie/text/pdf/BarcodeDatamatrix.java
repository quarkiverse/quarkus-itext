...
public class BarcodeDatamatrix {
    ...
    private static int getEncodation(byte[] text, int textOffset, int textSize, byte[] data, int dataOffset, int dataSize,
            int options, boolean firstMatch) {
        int e, j, k;
        int[] e1 = new int[6];
        if (dataSize < 0)
            return -1;
    ...
    public int generate(byte[] text, int textOffset, int textSize) {
        ...
        if (height == 0 || width == 0) {
            ...
            e = getEncodation(text, textOffset + extOut, textSize - extOut, data, extCount, last.dataSize - extCount, options,
                    false);
            if (e < 0) {
    ...
    /**
     * Sets the height of the barcode. If the height is zero it will be calculated. This height doesn't include the whitespace
     * border, if any.
     * <p>
    ...
    /**
     * Sets the width of the barcode. If the width is zero it will be calculated. This width doesn't include the whitespace
     * border, if any.
     * <p>
     * The allowed dimensions are (height, width):
     * <p>
     ...
     * <CODE>DM_RAW</CODE> - no encodation. The bytes provided are already encoded and will be added directly to the barcode,
     * using padding if needed. It assumes that the encodation state is left at ASCII after the last byte.<br>
     * <p>
     * One of:<br>
     * <CODE>DM_EXTENSION</CODE> - allows extensions to be embedded at the start of the text:
     * <p>
     * exxxxxx - ECI number xxxxxx<br>
     * m5 - macro 5<br>
     * m6 - macro 6<br>
     * f - FNC1<br>
     * saabbccccc - Structured Append, aa symbol position (1-16), bb total number of symbols (2-16), ccccc file identification
     * (0-64515)<br>
